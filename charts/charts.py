...
season_param = alt.param(name="Season",
    bind=alt.binding_select(options=[]), )

metric_param = alt.param(name="Metric", bind=alt.binding_select(options=[
        "goals_for_roll",
        "shots_roll",
...
team_filter_param = alt.param(name="TeamFilter", bind=alt.binding_select(options=["All"]), value="All")

homeaway_brush = alt.selection_interval(name="HomeAwayBrush", encodings=["x", "y"])

team_filter_logic = ("(TeamFilter != 'All') ? datum.team == TeamFilter : "
...
def chart_rolling(team_matches):
    ...
    rolling_chart = (
        ...
        .properties(
            width=800,
            height=400,
            title="Rolling Attacking Performance Over Time"
        )
    )
